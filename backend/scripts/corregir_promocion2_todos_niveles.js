// Corrige (o crea, si no existe) la inscripción en los 4 niveles del SFL para la Promoción 2,
...
//
// Por defecto busca el archivo en backend/data/reporte_sfl_promocion2.xlsx.
...
const rutaArchivo = rutaArgumento || path.join(__dirname, '../data/reporte_sfl_promocion2.xlsx');
...
const NIVELES = [
  { orden: 1, columna: 'SFL I', fecha: '2025-02-16' },
  { orden: 2, columna: 'SFL II', fecha: '2025-03-09' },
  { orden: 3, columna: 'SFL III', fecha: '2025-04-06' },
  { orden: 4, columna: 'SFL IV', fecha: '2025-06-15' }
];
...
async function main() {
  ...
  if (!fs.existsSync(rutaArchivo)) {
    console.error('No se encontró el archivo. Colócalo en backend/data/reporte_sfl_promocion2.xlsx, o pasa la ruta como argumento.');
    process.exit(1);
  }
    ...
    for (const { orden, fecha } of NIVELES) {
      ...
      if (anterior.ciclo === evento.ciclo_actual) {
        posiblesDobles.push(
          ...
          `El Excel indica Promoción 2 el ${fecha}. Revisar a mano — probablemente repitió el nivel.`
        );
        continue;
      }
