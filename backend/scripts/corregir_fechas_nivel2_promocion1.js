// Corrige la fecha de registro, ciclo (#1) y promoción (1) de la inscripción al Nivel II para
...
// resulta estar HOY activo en el ciclo en vivo de Nivel II (es decir, se re-registró
...
//   node scripts/corregir_fechas_nivel2_promocion1.js --aplicar
//
// Por defecto busca el archivo en backend/data/fecha_promo1_sfl2.xlsx.
...
const rutaArchivo = rutaArgumento || path.join(__dirname, '../data/fecha_promo1_sfl2.xlsx');
...
async function main() {
  ...
  if (!fs.existsSync(rutaArchivo)) {
    console.error('No se encontró el archivo. Colócalo en backend/data/fecha_promo1_sfl2.xlsx, o pasa la ruta como argumento.');
    process.exit(1);
  }

  const { rows: nivel2Rows } = await pool.query('SELECT id, ciclo_actual FROM eventos WHERE orden = 2');
  const nivel2 = nivel2Rows[0];
  ...
  for (const fila of datos) {
    ...
    if (!inscExistente.length) {
      sinInscripcionNivel2.push(`${participante.nombre_completo} (#${participante.id}) — no tiene inscripción de Nivel II para corregir.`);
      continue;
    }
    const anterior = inscExistente[0];

    // Protección: si esta persona está HOY activa en el ciclo en vivo, no la tocamos aquí.
    if (anterior.ciclo === nivel2.ciclo_actual) {
      posiblesDobles.push(
        `${participante.nombre_completo} (#${participante.id}, DNI ${participante.dni}) -> está ACTIVA hoy en el ciclo en vivo (#${nivel2.ciclo_actual}) de Nivel II. ` +
        `El Excel indica que también hizo Nivel II el ${fecha} (Promoción I). Revisar a mano — probablemente repitió el nivel.`
      );
      continue;
    }
    ...
    console.log(`[${participante.nombre_completo}] (#${participante.id}) -> Nivel II: fecha ${fecha}, ciclo ${CICLO}, promoción ${PROMOCION}, graduación ${fecha}`);
    if (aplicar) {
  ...
  if (sinInscripcionNivel2.length) {
    console.log(`\nSin inscripción de Nivel II (no se pudo corregir — ${sinInscripcionNivel2.length}):`);
    sinInscripcionNivel2.forEach(s => console.log('  - ' + s));
  }
  if (ambiguos.length) {
