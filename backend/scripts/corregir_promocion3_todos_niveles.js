// Corrige (o crea, si no existe) la inscripción en los 4 niveles del SFL para la Promoción 3,
...
//
// Por defecto busca el archivo en backend/data/reporte_sfl_promocion3.xlsx.
...
const rutaArchivo = rutaArgumento || path.join(__dirname, '../data/reporte_sfl_promocion3.xlsx');
...
const NIVELES = [
  { orden: 1, columna: 'Fecha SFL I', fecha: '2025-07-20' },
  { orden: 2, columna: 'Fecha SFL II', fecha: '2025-08-03' },
  { orden: 3, columna: 'Fecha SFL III', fecha: '2025-09-21' },
  { orden: 4, columna: 'Fecha SFL IV', fecha: '2025-10-05' }
];
...
async function main() {
  ...
  if (!fs.existsSync(rutaArchivo)) {
    console.error('No se encontró el archivo. Colócalo en backend/data/reporte_sfl_promocion3.xlsx, o pasa la ruta como argumento.');
    process.exit(1);
  }
    ...
    for (const { orden, fecha } of NIVELES) {
      ...
      if (anterior.ciclo === evento.ciclo_actual) {
        posiblesDobles.push(
          ...
          `El Excel indica Promoción 3 el ${fecha}. Revisar a mano — probablemente repitió el nivel.`
        );
        continue;
      }
