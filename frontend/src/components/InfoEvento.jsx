import { formatearRangoFechas, formatearFechaHora } from '../fechas';
...
export default function InfoEvento({ evento, compacto = false }) {
  ...
  return (
    <div className={compacto ? 'text-xs text-ink/50' : 'text-sm text-ink/60'}>
      {tieneFecha && (
        <p>📅 {formatearRangoFechas(evento.fecha_evento, evento.fecha_evento_fin)}{evento.hora_evento ? ` · ${evento.hora_evento}` : ''}{evento.lugar ? ` · ${evento.lugar}` : ''}</p>
      )}
      {tieneLimite && (
